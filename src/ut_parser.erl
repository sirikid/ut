...
'template'(Input, Index) ->
  p(Input, Index, 'template', fun(I,D) -> (p_seq([p_zero_or_more(p_choose([fun 'literals'/2, fun 'expression'/2])), p_not(p_anything())]))(I,D) end, fun(Node, _Idx) ->lists:append(Node) end).
...
'pct_encoded'(Input, Index) ->
  p(Input, Index, 'pct_encoded', fun(I,D) -> (p_seq([p_string(<<"%">>), p_charclass(<<"[0-9A-Za-z]">>), p_charclass(<<"[0-9A-Za-z]">>)]))(I,D) end, fun(Node, Idx) ->transform('pct_encoded', Node, Idx) end).

-spec 'expression'(input(), index()) -> parse_result().
'expression'(Input, Index) ->
  p(Input, Index, 'expression', fun(I,D) -> (p_seq([p_string(<<"{">>), fun 'operator'/2, fun 'variables'/2, p_string(<<"}">>)]))(I,D) end, fun(Node, _Idx) ->list_to_tuple(lists:droplast(tl(Node))) end).
